...
#include "PS2MouseHandler.h"
  ...
  //int esp = 0; Arduino Uno / Pro Mini...
int  esp = 40; //40 milis para ESP32 
...
void PS2MouseHandler::write(int data) {
  ...
  for (i=0; i < 8; i++) {
    ...
    while (!digitalRead(_clock_pin)) {;}delayMicroseconds(20); // compatibilizar com o esp32
    while (digitalRead(_clock_pin)) {;}delayMicroseconds(20); // compatibilizar com o esp32
  ...
  // wait for clock cycle
  while (!digitalRead(_clock_pin)) {;}delayMicroseconds(20); // compatibilizar com o esp32
  while (digitalRead(_clock_pin)) {;} delayMicroseconds(esp/2); // compatibilizar com o esp32
  pull_high(_data_pin); // release data line
  while (digitalRead(_data_pin)) {;} // wait for mouse to take over data line
  delayMicroseconds(esp/2); // compatibilizar com o esp32
...
void PS2MouseHandler::pull_low(int pin) {
  ...
  delayMicroseconds(esp); // compatibilizar com o esp32
}
